...
* 高&nbsp;&nbsp;&nbsp;&nbsp;林
** JBoss 产品化团队
** 微博: image:weibo.ico[] @JBoss高林

* &nbsp; &nbsp;

* 张建峰
** WildFly 核心开发人员
** 微博: image:weibo.ico[] @寻找一个苹果
...
[role="speaker"]
====
* zeroturnaround 就是　JRebel　开发的那个网站
** 他们从
*** 下载安装的难以程度
*** 实际的性能测量
*** 工具的支持程度
...
对于从事开源项目的人来说，看看哪些方面会影响到你的项目的推广

等进行比较。最终　JBoss　赢得了比赛。
大家有兴趣的可以上去看一下具体的比较内容
...
[role="speaker"]
====
* JBoss　是个社区，应用服务器也应该有个自己的名字。
** 使社区版和商业版分开，不容易混淆
* 提供了一个轻量级的可管理的运行时环境
** 什么都没有部署的情况下，启动　２　s
** 启动占用的内存非常少。可以运行在嵌入式的硬件平台上，比如之前　Jim　演示过的运行在　Raspberry　PI　上的　jboss　AS　７　的集群
** 提供了一个统一的管理层管理服务器的各个方面
*** 提供丰富的　API　来进行管理
**** Jason　Green　之前使用过　perl　脚本进行管理　JBoss　服务器，　非常适合使用脚本进行管理
* 对开发人员友好, 丰富的开发工具（现在的主流IDE　例如：　Eclipse，　netbeans，　IntelliJ　都支持）
** 有很多的快速开发例子，　
** 丰富的文档(非常适合初学　javaee　领域的同学)
* 完全开源，　如果你有问题，还可以自行调试，也可以贡献自己的代码
====
...
** job　xml　定义不会自动　batch，　用户可以在　servlet　或者　ejb　timer　里调用
*** batchlet　适合定期执行的面向　task　的
*** chunk 适合大型数据处理，比如：生成报表，分析日志，图片／视频处理等。
...
[role="speaker"]
====
* EE　下不推荐使用　SE　下的　java.lang.Thread, java.util.Timer
** 因为创建的　thread　托管了，　托管后的　thread　就不能很好的使用　EE　管理下的服务，比如　Transaction, security 等
** 很容易从　SE　移植
** 执行时的上下文，包括　事务／安全
** New 出来的　task　不能使用　cdi，但是一个task　可以是一个　CDI　的bean，　那么就可以使用　cdi　的　annotation　了。
...
[role="speaker"]
====
* 不仅仅是　JavaEE　full　features
** 例如　TorqueBox 使用　WildFly　的内核实现了　Ruby-on-Railes 的平台
...
[role="speaker"]
====
* 如果有意向转向　wildfly，　wildfly　社区也提供了大量的文档介绍如何进行迁移
====
...
NOTE: WildFly Document  - https://docs.jboss.org/author/display/WFLY8/Documentation
